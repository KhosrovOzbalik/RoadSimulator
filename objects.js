...
import {addCity, graph, grid, roads} from "./datas";
...
export function constructRoad(coords, object) {
    roads.push(new Road(coords, object));

    const buildingClone = object.fbxObject.clone();
    buildingClone.position.set(coords.x, 0, coords.y);
    scene.add(buildingClone);
    object.SetOccupying(coords, grid);
}